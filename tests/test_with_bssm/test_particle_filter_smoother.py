...
class TestParticleSmoother:
    """
    Test Particle Filter Smoother - ssm_nlg
    """
    ...
    def test_bspfilter_TFP_arexp(self):
        ...
        infer_result <- particle_smoother(model_nlg, particles = 200)
            """)
    ...
    def test_bspfilter_TFP_sinexp(self):
        ...
        infer_result <- particle_smoother(model_nlg, particles = 200)
        infer_result2 <- bootstrap_filter(model_nlg, particles = 200)
            """)
        r_result = ro.r("infer_result")
        r_result_filter = ro.r("infer_result2")
    ...
    def test_kffilter_TFP_mvmodel(self):
       ...
       set.seed(1)
        ...
        infer_result <- particle_smoother(model_nlg, particles = 2000)
            """)
        r_result = ro.r("infer_result")
