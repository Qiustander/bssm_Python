...
def _check_parameters(obs_len,
    ...
    state_mtx_noise = tf.convert_to_tensor(check_state_noise(state_noise, state_dim, obs_len),
                                           dtype=dtype)  # already 3 dim
    ...
    if input_obs.shape[-1] == obs_len or obs_mtx_noise.shape[-1] == obs_len:
        input_obs = tf.repeat(input_obs[..., tf.newaxis], obs_len, axis=-1) \
            if input_obs.shape[-1] != obs_len else input_obs
        obs_mtx_noise = tf.repeat(obs_mtx_noise[..., tf.newaxis], obs_len, axis=-1) \
