...
def kalman_smoother(ssm_model, observations):
    ...
    smoothed_means, smoothed_covs = backward_smoothing_pass(ssm_model.transition_fn,
                                                            filtered_means, filtered_covs,
...
def backward_smoothing_pass(transition_fn, filtered_means, filtered_covs,
    ...
    Args:
      transition_dist: callable derivative of the transition function
...
def build_backward_pass_step(transition_fn):
    """Build a callable that perform one step for backward smoothing.

  Args:
    transition_fn: callable taking the mu_t|t
