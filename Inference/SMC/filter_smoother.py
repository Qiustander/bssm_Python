...
def filter_smoother(ssm_model,
  ...
  """
    with tf.name_scope(name or 'filter_smoother'):
        if seed is None:
            seed = samplers.sanitize_seed(seed, name='filter_smoother')
        pf_seed, smoother_seed = samplers.split_seed(
        ...
        # weighted_trajectories = one_trajectory(particles=particles,
        #                                        weights=log_weights,
