...
result_fast <- fast_smoother(model_temp)
z <-matrix(1, 2, 1)
h<-matrix(c(0.15,0.05,0, 0.05), 2, 2)

esty <- result_fast%*%t(z)+ matrix( rnorm(length(model_temp$y[,1])*2,mean=0,sd=1), length(model_temp$y[,1]), 2)%*%h 
# ts.plot(cbind(result_fast), col = 1:1)
